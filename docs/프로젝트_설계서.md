...
├── App.vue
├── router/
│   └── index.js                 # 라우팅 설정 (홈/게시판/상세/작성/대시보드)
│
├── components/
│   ├── layout/
│   │   ├── AppHeader.vue        # 로고 + 카테고리 네비게이션 + 검색
│   │   └── ChatbotWidget.vue    # 챗봇 플로팅 버튼 + 대화창 (전역 배치)
│   │
│   ├── home/
│   │   ├── HomeBanner.vue       # 서비스 소개 배너
│   │   ├── CategoryCard.vue     # 카테고리 카드 (props: icon, label, category)
│   │   └── DashboardSummary.vue # 홈 화면용 미니 통계 요약
│   │
│   ├── board/
│   │   ├── PostSearchBar.vue    # 검색창 + 글쓰기 버튼
│   │   ├── PostTable.vue        # 게시글 목록 테이블
│   │   ├── PostPagination.vue   # 페이지네이션
│   │   ├── PostDetail.vue       # 게시글 상세 (제목/본문/조회수/좋아요)
│   │   ├── PostForm.vue         # 작성/수정 공용 폼
│   │   └── PasswordConfirmModal.vue  # 수정/삭제 시 비밀번호 확인 모달
│   │
│   └── dashboard/
│       ├── CategoryChart.vue    # 카테고리별 통계 차트
│       ├── DistrictChart.vue    # 자치구별 통계 차트
│       └── TopPostsRanking.vue  # 인기 게시글 랭킹
│
├── views/
│   ├── HomeView.vue             # ① 홈 화면
...
│   └── DashboardView.vue        # ⑤ 데이터 시각화 대시보드
│
├── composables/
│   ├── usePosts.js              # localStorage 게시글 CRUD 로직 (get/add/update/delete)
│   └── useChatbot.js            # OpenAI API 호출 로직
│
└── utils/
    └── placeData.js             # 서울 JSON(관광지/문화시설/레포츠/쇼핑) 로드 및 가공
```
